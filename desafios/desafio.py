...
class Carro(Veiculo):
    ...
    def __str__(self):
        return f"Marca: {self.marca}, Modelo: {self.modelo}, Ligado: {self._ligado}, Portas: {self.portas}"
...
class Moto(Veiculo):
    ...
    # 6. Implemente o Método Especial str na Classe Filha: Adicione um método especial str à classe Moto que estenda o método da classe pai (Veiculo) e inclua a informação sobre o tipo da moto.
    def __str__(self):
        return f"Marca: {self.marca}, Modelo: {self.modelo}, Ligado: {self._ligado}, Tipo: {self.tipo}"

# 7. Crie um Arquivo Main (main.py): Crie um arquivo chamado main.py no mesmo diretório que suas classes. - ok.

# 8. Importe e Instancie Objetos: No arquivo main.py, importe as classes Carro e Moto. Em seguida, crie três instâncias de Carro e Moto com diferentes marcas, modelos, quantidade de portas e tipos. - ok.
    
# 9. Exiba as Informações: Para cada instância, imprima no console as informações utilizando o método str. - ok.